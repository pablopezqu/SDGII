/** File Name: tecldo.c
  * Description: programa que implementa el teclado matricial 4x4
 */
#include "teclado.h"

TipoTeclado teclado; //Variable TipoTeclado con la que operamos en el programa. Es compartida con el programa principal
int flags=0;	//variable flags donde activaremos los flags asociados a la reproducción de efectos. Es compartida con el programa principal
int debounceTime[NUM_ROWS] = {0,0,0,0}; // Timeout to avoid bouncing after pin event


//Matriz con los caracteres del teclado matricial en el mismo orden
char teclado4x4[4][4] = {
	{'1', '2', '3', 'A'},
	{'4', '5', '6', 'B'},
	{'7', '8', '9', 'C'},
...
//--------------------------------------------------------------------
// FUNCIONES DE ATENCIÓN A LAS INTERRUPCIONES GENERADAS POR LAS FILAS
//--------------------------------------------------------------------
void row_1_isr (void){
	...
	flags |= FLAG_KEY_PRESSED;		//Activa el flag de que se ha pulsado una tecla
...
//-----------------------------------------
// FUNCIONES DE EXCITACIÓN DE LAS COLUMNAS
//-----------------------------------------
void col_1 (fsm_t* this){
	...
	tmr_startms((tmr_t*)(p_teclado->tmr_duracion_columna),COL_REFRESH_TIME);	//Inicias el temporizador. Cuando acabe este temporizador, se excitará otra col.
}
...
void process_key (fsm_t* this) {
	...
	switch(p_teclado->teclaPulsada.col){		//Según la columna que se ha pulsado, planteamos varios casos
		...
		case COL_4:
			...
			else if(teclado4x4[p_teclado->teclaPulsada.row][p_teclado->teclaPulsada.col]=='*'){
				piLock(PLAYER_FLAGS_KEY);
				flags_player |= FLAG_START_IMPACTO;
				piUnlock(PLAYER_FLAGS_KEY);
			}
			else if(teclado4x4[p_teclado->teclaPulsada.row][p_teclado->teclaPulsada.col]=='2'){
				piLock(SYSTEM_FLAGS_KEY);
				flags_juego |= FLAG_JOYSTICK_UP;
				piUnlock(SYSTEM_FLAGS_KEY);
			}
			else if(teclado4x4[p_teclado->teclaPulsada.row][p_teclado->teclaPulsada.col]=='4'){
							piLock(SYSTEM_FLAGS_KEY);
							flags_juego |= FLAG_JOYSTICK_UP;
							piUnlock(SYSTEM_FLAGS_KEY);
			}
			...
			else if(teclado4x4[p_teclado->teclaPulsada.row][p_teclado->teclaPulsada.col]=='6'){
							piLock(SYSTEM_FLAGS_KEY);
							flags_juego |= FLAG_JOYSTICK_UP;
							piUnlock(SYSTEM_FLAGS_KEY);
			}
...
/*
 * Función de atención a la interrupción que produce el temporizador al acabar indicando de que se debe pasar a explorar otra columna
 */
...
/*
 * Función de inicialización del subsistema del teclado
 */
int initialize(TipoTeclado *p_teclado) {
	...
	//La función wiringPiISR(...) permite definir una fuente de interrupción en un pin del GPIO y asociarla una función de atención a esa interrupción
