...
void InicializaTorreta (TipoTorreta *p_torreta) {
	...
	if(p_torreta->servo_x.posicion > p_torreta->servo_x.maximo ){		//Pos maxima y minima
		p_torreta->servo_x.posicion = p_torreta->servo_x.maximo;
	}

	if(p_torreta->servo_y.posicion > p_torreta->servo_y.maximo ){		//Pos maxima y minima
	...
	softPwmCreate (SERVO_PIN_X, p_torreta->servo_x.inicio , SERVO_PWM_RANGE);
...
void MueveTorretaIzquierda (fsm_t* this) {
	...
	piLock(STD_IO_BUFFER_KEY);
	serialPrintf("Moviendo a la izquierda\n");
	piUnlock(STD_IO_BUFFER_KEY);
...
void MueveTorretaDerecha (fsm_t* this) {
	...
	piLock(STD_IO_BUFFER_KEY);
	serialPrintf("Moviendo a la derecha\n");
	piUnlock(STD_IO_BUFFER_KEY);
